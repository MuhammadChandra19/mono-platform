...
import { toServiceError, toServiceException, toServiceSuccess } from '@/utils/helpers/serviceResult';
import { AuthServiceApiImpl, LoginResponse, ServiceResult } from '@packages/openapigen/src';
...
const authService = ({ identityUsecase, permissionUsecase, maker }: Deps): AuthServiceApiImpl => {
  const authServiceLogin = async (c: Context, params: AuthServiceLoginRequest): Promise<ServiceResult<LoginResponse>> => {
    try {
      const result = await identityUsecase.getUserByEmail(params.loginRequest.email!);
      if(!result.ok) {
        return toServiceError<LoginResponse>(result.error);
      }

      const matchPassword = comparePassword(params.loginRequest.password!, result.data.password!);
      if(!matchPassword) {
      ...
      const rolesAndPermissionsResult = await permissionUsecase.getUserPermissions(result.data.id);
      if(!rolesAndPermissionsResult.ok) {
        return toServiceError<LoginResponse>(rolesAndPermissionsResult.error);
      }

      const stringPermission = rolesAndPermissionsResult.data.map((perm) => perm.permissionId).join(',');
      ...
      c.set('set-cookie', `refreshToken=${refreshToken}; HttpOnly; Path=/; Max-Age=${7 * 24 * 60 * 60}`);
      c.set('set-cookie', `accessToken=${accessToken}; HttpOnly; Path=/; Max-Age=${15 * 60}`);
